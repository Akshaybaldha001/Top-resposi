...
function ManageProduct() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:4000/bestproduct")
      .then((res) => {
        setBlogs(res.data);
      })
      .catch((error) => {
          ...
          <div className="row">
            ...
            <table className="table table-responsive table-bordered w-full">
              ...
              <tbody>
                {blogs &&
                  blogs.map((item) => (
                    <tr key={item.id} className="border-b border-black-200">
                      <td className="px-4 py-2 border-r border-black-200">
                        {item.title}
                      </td>
                      <td className="px-4 py-2 border-r border-black-200">
                        <img
                      ...
                      <td className="px-4 py-2">
                        <div className="flex justify-center">
                          ...
                          <button
                            className="btn bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ms-2"
